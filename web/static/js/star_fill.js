class Fill_Star
{
    constructor()
    {
        ...
        this.number = this._get_two_numbers($('.mov_rating_text'))
    }
    ...
    _get_two_numbers(selector)
    {
        const integer_part = selector.text()
        const fractional_part = (integer_part % 1).toFixed(1)
        return {integer:Math.ceil(integer_part - fractional_part), fraction: Number(fractional_part) * 100}
    }
    ...
    _add_half_color()
    {
        $(this.half_star_element.element).children('path').
        attr('fill', `url(#grad_${this.half_star_element.index})`)
    }
}
